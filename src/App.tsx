      ...
      <main id="workspace">
        ...
        {editingPost && <div className="editor-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) closeEditor() }}>
          <form className="post-editor" onSubmit={savePost} role="dialog" aria-modal="true" aria-labelledby="editor-title">
            ...
            <label className="field field--wide"><span>Тема публикации</span><input name="title" autoComplete="off" aria-invalid={Boolean(formErrors.title)} aria-describedby={formErrors.title ? 'error-title' : undefined} value={editingPost.title} onChange={(event) => setEditingPost({ ...editingPost, title: event.target.value })} placeholder="Что аудитория должна понять…" />{formErrors.title && <small className="field-error" id="error-title">{formErrors.title}</small>}</label>
            <label className="field field--wide"><span>Хук</span><textarea name="hook" autoComplete="off" rows={3} aria-invalid={Boolean(formErrors.hook)} aria-describedby={formErrors.hook ? 'error-hook' : undefined} value={editingPost.hook} onChange={(event) => setEditingPost({ ...editingPost, hook: event.target.value })} placeholder="Первый тезис, который остановит скролл…" />{formErrors.hook && <small className="field-error" id="error-hook">{formErrors.hook}</small>}</label>
            <label className="field"><span>Рубрика</span><input name="pillar" autoComplete="off" aria-invalid={Boolean(formErrors.pillar)} value={editingPost.pillar} onChange={(event) => setEditingPost({ ...editingPost, pillar: event.target.value })} />{formErrors.pillar && <small className="field-error">{formErrors.pillar}</small>}</label>
            <label className="field"><span>Формат</span><input name="format" autoComplete="off" aria-invalid={Boolean(formErrors.format)} value={editingPost.format} onChange={(event) => setEditingPost({ ...editingPost, format: event.target.value })} />{formErrors.format && <small className="field-error">{formErrors.format}</small>}</label>
            <label className="field"><span>Дата</span><input name="scheduledDate" type="date" autoComplete="off" aria-invalid={Boolean(formErrors.scheduledDate)} value={editingPost.scheduledDate} onChange={(event) => setEditingPost({ ...editingPost, scheduledDate: event.target.value })} />{formErrors.scheduledDate && <small className="field-error">{formErrors.scheduledDate}</small>}</label>
            <label className="field"><span>Время · Тбилиси</span><input name="scheduledTime" type="time" autoComplete="off" aria-invalid={Boolean(formErrors.scheduledTime)} value={editingPost.scheduledTime} onChange={(event) => setEditingPost({ ...editingPost, scheduledTime: event.target.value })} />{formErrors.scheduledTime && <small className="field-error">{formErrors.scheduledTime}</small>}</label>
            <label className="field"><span>Аккаунт</span><select name="accountId" autoComplete="off" value={editingPost.accountId} onChange={(event) => setEditingPost({ ...editingPost, accountId: event.target.value })}>{accounts.filter((account) => account.state === 'active').map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></label>
            <fieldset className="field channel-field" aria-invalid={Boolean(formErrors.channels)}><legend>Каналы</legend>{(['Facebook', 'Instagram'] as Channel[]).map((channel) => <label key={channel}><input name="channels" type="checkbox" checked={editingPost.channels.includes(channel)} onChange={(event) => setEditingPost({ ...editingPost, channels: event.target.checked ? [...editingPost.channels, channel] : editingPost.channels.filter((item) => item !== channel) })} />{channel}</label>)}{formErrors.channels && <small className="field-error">{formErrors.channels}</small>}</fieldset>
            <label className="field field--wide"><span>Текст Facebook</span><textarea name="facebookCaption" autoComplete="off" rows={4} value={editingPost.facebookCaption} onChange={(event) => setEditingPost({ ...editingPost, facebookCaption: event.target.value })} placeholder="Полный текст для Facebook…" /></label>
            <label className="field field--wide"><span>Текст Instagram</span><textarea name="instagramCaption" autoComplete="off" rows={4} value={editingPost.instagramCaption} onChange={(event) => setEditingPost({ ...editingPost, instagramCaption: event.target.value })} placeholder="Полный текст для Instagram…" /></label>
            <label className="field"><span>CTA</span><input name="cta" autoComplete="off" value={editingPost.cta} onChange={(event) => setEditingPost({ ...editingPost, cta: event.target.value })} placeholder="Получить аудит…" /></label>
            <label className="field"><span>HTTPS-ссылка</span><input name="destinationUrl" type="url" inputMode="url" autoComplete="off" aria-invalid={Boolean(formErrors.destinationUrl)} value={editingPost.destinationUrl} onChange={(event) => setEditingPost({ ...editingPost, destinationUrl: event.target.value })} placeholder="https://rocket-peak.com/…" />{formErrors.destinationUrl && <small className="field-error">{formErrors.destinationUrl}</small>}</label>
